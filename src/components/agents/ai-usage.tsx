...
export function AiUsageCard() {
  ...
  const chartData =
    data?.daily.map((d) => ({ day: format(parseISO(d.date), "d 'de' MMM", { locale: ptBR }), Tokens: d.tokens })) ??
    [];
      ...
      <CardContent className="space-y-5">
        {loading || !data ? (
          ...
          <div className="flex flex-col items-center justify-center gap-2 py-10 text-center text-sm text-muted-foreground">
            ...
            <p className="text-xs">
              Isso é preenchido conforme o assistente cria rascunhos e responde automaticamente.
            </p>
          </div>
        ) : (
          <>
            ...
            {data.truncated && (
              <p className="text-xs text-muted-foreground">
                Mostrando uma janela parcial. O uso é alto o suficiente para que só os
                registros mais recentes sejam resumidos aqui.
              </p>
            )}
          </>
        )}
