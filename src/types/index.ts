...
export interface Contact {
  ...
  user_id: string;
  account_id: string;
  /** Nullable since migration 039 — a contact reached only via a
   *  WhatsApp username (no phone shared) has `wa_user_id` instead.
  ...
  /** WhatsApp business-scoped user id (BSUID) — set when this contact
   *  was identified via Meta's usernames feature instead of (or in
   *  addition to) a phone number. Migration 039. */
  wa_user_id?: string | null;
  /** Customer's optional WhatsApp username/display handle, if adopted.
   *  Migration 039. */
